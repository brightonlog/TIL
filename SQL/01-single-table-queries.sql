-- Active: 1762226659156@@127.0.0.1@3306
-- 1. 기본 SELECT 문제
...
--왜 grpu by와 where을 같이 쓰면 에러가 날까?
-- 과일 가게에 가서 평균 5천원 미만 과일을 담아주세요
-- 1단계 : 과일 종류 별로 group by
-- 2단계 : 각 과일 그룹 (과일 종류)의 평균 가격 계산
-- 3단계 : 마지막으로 평균 가격이 5천원 미만인 그룹을 선택해서 담는다
-- GROUP BY 하기 전에 WHERE 해버리면 오류가 나기 때문임

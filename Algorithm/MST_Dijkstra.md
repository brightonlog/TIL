...
**사이클**이란 그래프에서 3개 이상의 노드가 있을 때, **간선을 한 번씩만 거쳐서 출발했던 지점으로 다시 되돌아올 수 있는 경로**를 의미합니다.

---

## 🚗 다익스트라 (Dijkstra) 알고리즘

하나의 시작 정점에서 출발하여, 다른 **모든 정점**까지의 **최단 경로**를 구하는 알고리즘입니다.

### 다익스트라를 왜 쓸까?

-   **모든 도착지의 최소 비용 계산**: 시작점으로부터 다른 모든 노드까지의 최단 거리를 **한 번의 실행으로** 모두 알 수 있습니다. 결과는 `result` 배열에 저장됩니다.
-   **효율적인 속도**: **우선순위 큐(힙)**를 사용하여 가장 누적 거리가 짧은 노드를 먼저 방문하기 때문에 매우 효율적입니다. 시간 복잡도는 보통 **$O(E \log V)$** (E: 간선 수, V: 정점 수)로 매우 빠릅니다.
...
def dijkstra(start):
    n = len(MAP) # 노드의 개수
    # result: 시작점으로부터 각 노드까지의 최단 거리를 저장할 리스트
    result = [float('inf')] * n
    result[start] = 0 # 시작 노드의 거리는 0
    
    # 우선순위 큐(pq) 초기화: (누적 비용, 현재 노드)
    q = [(0, start)]

    while q: # 큐가 빌 때까지 반복
        # 1. 누적 비용이 가장 작은 노드 정보를 꺼냄
        price, now = heapq.heappop(q)

        # 2. 이미 더 짧은 경로가 있다면 건너뛰기
        if result[now] < price:
            continue
        ...
        for i in range(n):
            ...
            price_sum = price + next_price # 시작점부터 다음 노드까지의 총 누적 비용
            
            # 4. 기존 경로보다 더 짧은 경로를 발견하면 갱신하고 큐에 추가
...
# 출력 결과: 0 15 20 22 22 23
```
## ✅ 오늘의 핵심 요약

1.  **다익스트라 알고리즘**은 한 정점에서 **다른 모든 정점**까지의 **최단 경로**를 찾는 가장 효율적인 방법 중 하나입니다.
2.  **우선순위 큐(힙)**를 사용하여, 항상 **누적 거리가 가장 짧은 경로**를 먼저 탐색하는 탐욕적인 방식으로 동작합니다.
3.  `result` 배열을 두어, 각 노드까지의 최단 거리를 계속 갱신해나가는 것이 알고리즘의 핵심입니다.
